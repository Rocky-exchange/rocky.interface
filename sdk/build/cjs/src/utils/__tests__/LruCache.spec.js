...
(0, vitest_1.describe)("LruCache", () => {
    ...
    (0, vitest_1.it)("should delete a key and remove it from recent keys", () => {
        const cache = new LruCache_1.LRUCache(2);
        cache.set("a", 1);
        cache.set("b", 2);
        cache.delete("a");
        (0, vitest_1.expect)(cache.has("a")).toBe(false);
        (0, vitest_1.expect)(cache.get("a")).toBeUndefined();
        cache.set("c", 3);
        (0, vitest_1.expect)(cache.has("b")).toBe(true);
        (0, vitest_1.expect)(cache.has("c")).toBe(true);
    });
    (0, vitest_1.it)("should handle deletion of non-existent key", () => {
        const cache = new LruCache_1.LRUCache(2);
        cache.set("a", 1);
        cache.delete("x");
        (0, vitest_1.expect)(cache.has("a")).toBe(true);
