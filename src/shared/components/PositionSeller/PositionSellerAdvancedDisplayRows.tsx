...
export function PositionSellerAdvancedRows({ triggerPriceInputValue, slippageInputId, gasPaymentParams, isX10000Mode }: Props) {
  ...
  return (
    <ExpandableRow title={t`Execution Details`} open={open} onToggle={setOpen} contentClassName="flex flex-col gap-14">
      ...
      {!isX10000Mode && <TradeFeesRow {...fees} feesType="decrease" />}
      {!isX10000Mode && <NetworkFeeRow executionFee={executionFee} gasPaymentParams={gasPaymentParams} />}
      ...
      {!isTwap && (
        <>
          ...
          <SyntheticsInfoRow
            label={
              <Tooltip
                handle={<Trans>Collateral ({isX10000Mode ? "USDT" : position?.collateralToken?.symbol})</Trans>}
                position="top-start"
