...
import { shouldUseApiTrades, useAccountTrades } from "@/modules/cex/lib/api";
...
export function TradeHistoryRow({ minCollateralUsd, tradeAction, shouldDisplayAccount, showDebugValues }: Props) {
  ...
  const { trades: apiTrades } = useAccountTrades(chainId, account);

  // In x10000 mode, find the original API trade data by id
  const originalApiTrade = useMemo(() => {
    if (!isX10000Mode || !apiTrades) return undefined;
    return apiTrades.find((t) => t.id === tradeAction.id);
  }, [isX10000Mode, apiTrades, tradeAction.id]);
  ...
  }, [isSwapOrderType(tradeAction.orderType!) && marketsInfoData, minCollateralUsd.toString(), tradeAction.id, originalApiTrade]);
