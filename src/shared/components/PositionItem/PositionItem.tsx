...
import { calculateDisplayDecimals, formatBalanceAmount, formatDeltaUsd, formatPercentage, formatUsd } from "lib/numbers";
  ...
  function renderLarge() {
    ...
    return (
      <TableTr hoverable={true} data-qa={qaAttr}>
        ...
        {/* Leverage column is shown only in x10000 mode */}
        {isX10000Mode && (
          <TableTd>
            <span className="numbers">{formatLeverage(p.position.leverage) || "..."}</span>
          </TableTd>
        )}
        {/* Amount column is shown only in x10000 mode */}
        {isX10000Mode && (
          <TableTd>
            <div className="numbers whitespace-nowrap">
              {(() => {
                const originalAmount = (p.position as any).originalAmount;
                const originalSymbol = (p.position as any).originalSymbol;
                if (!originalAmount || !originalSymbol) return "-";

                // Extract base symbol from trading pair (e.g., "BTCUSDT" -> "BTC")
                const baseSymbol = originalSymbol.replace(/USDT$/i, "") || originalSymbol;
                const amountNum = parseFloat(originalAmount);

                return `${amountNum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })} ${baseSymbol}`;
              })()}
            </div>
          </TableTd>
        )}
        {/* Net Value column is hidden in x10000 mode */}
        {!isX10000Mode && (
          <TableTd>
            {/* netValue */}
        ...
        <TableTd>
          ...
          {p.position.isOpening ? (
            ...
            <span className="numbers whitespace-nowrap">
        ...
        <TableTd>
          ...
          {p.position.isOpening ? (
            ...
            <div className="flex flex-col gap-2">
              <div
                className={cx("text-body-small numbers", {
                  positive: displayedPnl > 0,
                  negative: displayedPnl < 0,
                  muted: displayedPnl == 0n,
                })}
              >
                {formatDeltaUsd(displayedPnl)}
              </div>
              <div
                className={cx("text-body-small numbers", {
                  positive: displayedPnlPercentage > 0,
                  negative: displayedPnlPercentage < 0,
                  muted: displayedPnlPercentage == 0n,
                })}
              >
                {formatPercentage(displayedPnlPercentage, { signed: true, displayDecimals: 2 }) || "..."}
              </div>
            </div>
          )}
        </TableTd>
    ...
    return (
      <AppCard dataQa="position-item">
        ...
        <AppCardSection>
          ...
          {/* Amount is shown only in x10000 mode */}
          {isX10000Mode && (
            <div className="App-card-row">
              <div className="font-medium text-typography-secondary">
                <Trans>Amount</Trans>
              </div>
              <div className="numbers break-words">
                {(() => {
                  const originalAmount = (p.position as any).originalAmount;
                  const originalSymbol = (p.position as any).originalSymbol;
                  if (!originalAmount || !originalSymbol) return "-";
          ...
          {/* Net Value is hidden in x10000 mode */}
          {!isX10000Mode && (
            <div className="App-card-row">
              <div className="font-medium text-typography-secondary">
          ...
          {/* Collateral is hidden in x10000 mode */}
          {!isX10000Mode && (
            <div className="App-card-row">
              <div className="font-medium text-typography-secondary">
