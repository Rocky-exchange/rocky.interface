...
import { useZtdxOrderSubmit, shouldUseApiOrderSubmit } from "@/modules/cex/lib/api";
...
export function useTradeboxTransactions({ setPendingTxns }: TradeboxTransactionsProps) {
  ...
  const { submitOrder: submitApiOrder, isApiEnabled, isReady: isApiReady } = useZtdxOrderSubmit();
  ...
  const onSubmitOrderViaApi = useCallback(async () => {
    ...
    const primaryOrder = primaryCreateOrderParams?.[0];
    if (!primaryOrder) {
      helperToast.error(t`No order to submit`);
      return Promise.reject();
    }

    // Get symbol from market info (e.g., "ETH-USD")
    const symbol = `${marketInfo.indexToken.symbol}-USD`;

    // Determine size based on order type
  ...
  // Choose order submission path based on Feature Flag
  const onSubmitOrder = useCallback(async () => {
    // Use API submission if enabled and ready (user authenticated)
    if (useApiOrderSubmit && isApiReady && !isSwap) {
      return onSubmitOrderViaApi();
    }
    // Fall back to on-chain submission
    return onSubmitOrderOnChain();
  }, [useApiOrderSubmit, isApiReady, isSwap, onSubmitOrderViaApi, onSubmitOrderOnChain]);
