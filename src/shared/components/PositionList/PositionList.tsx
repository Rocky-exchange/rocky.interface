  ...
  return (
    <div className="flex grow flex-col">
      ...
      {!isMobile && (
        ...
        >
          <Table className={isX10000Mode ? "!w-[max(100%,1100px)] table-fixed" : "!w-[max(100%,900px)] table-fixed"}>
            <thead className="text-body-medium">
              <TableTheadTr>
                <TableTh className={isX10000Mode ? "w-[14%]" : "w-[18%]"}>
                  <Trans>Position</Trans>
                </TableTh>
                <TableTh className={isX10000Mode ? "w-[8%]" : "w-[10%]"}>
                  <Trans>Size</Trans>
                </TableTh>
                {isX10000Mode && (
                  <TableTh className="w-[6%]">
                    <Trans>Leverage</Trans>
                  </TableTh>
                )}
                {/* Quantity column is shown only in x10000 mode */}
                {isX10000Mode && (
                  <TableTh className="w-[10%]">
                    <Trans>Quantity</Trans>
                  </TableTh>
                )}
                {/* Net Value column header is hidden in x10000 mode */}
                {!isX10000Mode && (
                  <TableTh className="w-[14%]">
                    <Trans>Net Value</Trans>
                  </TableTh>
                )}
                <TableTh className={isX10000Mode ? "w-[9%]" : "w-[14%]"}>
                  {isX10000Mode ? <Trans>Margin</Trans> : <Trans>Collateral</Trans>}
                </TableTh>
                <TableTh className={isX10000Mode ? "w-[11%]" : "w-[10%]"}>
                  <Trans>Entry Price</Trans>
                </TableTh>
                <TableTh className={isX10000Mode ? "w-[11%]" : "w-[10%]"}>
                  <Trans>Mark Price</Trans>
                </TableTh>
                <TableTh className={isX10000Mode ? "w-[9%]" : "w-[10%]"}>
                  <Trans>Liq. Price</Trans>
                </TableTh>
                <TableTh className={isX10000Mode ? "w-[10%]" : "w-[12%]"}>
                  <TooltipWithPortal
                    variant="iconStroke"
                    content={
                      <>
                        <Trans>Mark price is used to estimate unrealized PNL.</Trans>
                        <br />
                        <Trans>Only trade prices are used for realized PNL.</Trans>
                      </>
                    }
                  >
                    <Trans>PNL (ROE %)</Trans>
