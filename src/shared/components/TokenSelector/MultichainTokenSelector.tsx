...
export function MultichainTokenSelector({
  ...
  try {
    ...
    // tokenAddress may be stale after a chain switch; ignore and render nothing
  }
  ...
  useEffect(() => {
    if (isModalVisible) {
      ...
      // In x10000 mode, always show "pay" tab (Available to Pay list)
        ...
        headerContent={
          isConnected === false ? null : (
            <div className="pb-12">
              ...
              />
              {/* Hide tabs in x10000 mode - only show Available to Pay list */}
              {!isX10000ModeActive() && (
                <>
        ...
        {selectedTokenLabel || (
          <span className="inline-flex items-center">
            <TokenIcon
              ...
              symbol={token.symbol}
              displaySize={20}
              chainIdBadge={isX10000ModeActive() ? undefined : isGmxAccount ? 0 : undefined}
            />
            {token.symbol}
          </span>
        )}
...
function useAvailableToTradeTokenList({
  ...
  return useMemo(() => {
    ...
    // For x10000 mode, only show account balance (from API), not wallet balance
    ...
    for (const token of Object.values(tokensData ?? (EMPTY_OBJECT as TokensData))) {
      ...
      // Only include wallet balance if NOT in x10000 mode
      if (!isX10000Mode && token.walletBalance !== undefined && srcChainId === undefined) {
          ...
          >
            <div className="flex items-center gap-16">
              <TokenIcon
                ...
                className="size-40"
                displaySize={40}
                chainIdBadge={isX10000Mode ? undefined : token.isGmxAccount ? 0 : chainId}
              />
