...
import { requestWithdraw, confirmWithdraw, isAuthenticated } from "@/modules/cex/lib/api/custom/client";
import { getX10000ZtdxVaultAddress, getX10000UsdtAddress } from "config/custom/contracts";
...
export const WithdrawalView = () => {
  ...
  // X10000 mode check - must be defined before any useEffect that uses it
  const isX10000Mode = isX10000ModeActive();

  // In x10000 mode, ensure USDT is selected by default
  useEffect(() => {
    if (isX10000Mode && chainId && !selectedTokenAddress) {
      const usdtAddress = getX10000UsdtAddress(chainId);
      if (usdtAddress) {
        setSelectedTokenAddress(usdtAddress);
      }
    }
  }, [isX10000Mode, chainId, selectedTokenAddress, setSelectedTokenAddress]);
  const { walletClient } = useWallet();
  const publicClient = usePublicClient({ chainId });
  // Only call useZtdxUserBalances in x10000 mode to avoid state updates during render
  const balancesResult = useZtdxUserBalances(
    isX10000Mode ? { refreshInterval: 10000 } : undefined
  );
  const mutateBalances = isX10000Mode ? balancesResult.mutate : undefined;
  ...
  }, [chainId, tokensData, isX10000Mode]);
  ...
  const handleWithdraw = async () => {
    ...
    // In x10000 mode, use API-based withdrawal with on-chain contract call
    if (isX10000Mode) {
      if (!chainId || !walletClient || !publicClient || !inputValue || !inputAmountUsd || inputAmountUsd === 0n) {
      ...
      try {
        ...
        console.log("[WithdrawalView] Contract call parameters:", {
          vaultAddress,
        ...
        try {
          console.log("[WithdrawalView] Simulating contract call...");
          ...
          console.log("[WithdrawalView] Contract simulation successful, proceeding with transaction...");
    ...
    // Original blockchain-based withdrawal logic for non-x10000 mode
  ...
  } else if (isX10000Mode) {
    // In x10000 mode, skip express transaction validation checks
