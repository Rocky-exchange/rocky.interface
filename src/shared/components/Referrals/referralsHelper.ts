...
export function isRecentReferralCodeNotExpired(referralCodeInfo, hasIndexer = true) {
  // For chains without indexer, keep codes permanently
  if (!hasIndexer) {
    return true;
  }
...
export function getTwitterShareUrl(referralCode) {
  const message = ["Trying out trading, up to 50x leverage on $BTC, $ETH 📈", "For fee discounts use:"];
