...
export function AppRoutes() {
  ...
  // Earn page analytics commented out
  ...
  return (
    <>
      <div className="App flex h-full w-full overflow-hidden">
        {!isLanding && !isLighterTrade && (
          <>
            {/* 左侧 SideNav 只挂载一次，避免切换路由时 logo 抖动 */}
            <div className="z-30 hidden shrink-0 p-2 lg:block">
              <SideNav />
            </div>
          </>
        )}
        <div className="flex h-full grow flex-col min-w-0 overflow-hidden">
      ...
      <NotifyModal />
      {shouldShowBotAssistant && <BotAssistant />}
    </>
  );
}
