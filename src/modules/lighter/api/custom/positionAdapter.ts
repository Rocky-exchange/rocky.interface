...
import { normalizeCantonAccountKey, stableHexHash } from "./cantonAdapterUtils";
 ...
 */
function generateContractKey(account: string, marketAddress: string, collateralTokenAddress: string, isLong: boolean): string {
  return stableHexHash(`${account}:${marketAddress}:${collateralTokenAddress}:${isLong}`);
}
...
export function convertApiPositionToSdk(
  ...
  const normalizedAccount = normalizeCantonAccountKey(account);
