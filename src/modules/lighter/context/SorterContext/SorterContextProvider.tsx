...
const DEFAULT_SORTER_STATE: SorterState = {
  ...
  "trade-markets-dropdown": { orderBy: "unspecified", direction: "unspecified", isDefault: true },
};
...
export function SorterContextProvider({ children }: PropsWithChildren) {
  ...
  const setConfig = useCallback(
    (key: SorterKey, config: React.SetStateAction<SorterConfig<any>>) => {
      ...
      if (newState[key]) {
        newState = updateByKey(newState, key, typeof config === "function" ? config(newState[key]) : config) as SorterState;
      } else {
