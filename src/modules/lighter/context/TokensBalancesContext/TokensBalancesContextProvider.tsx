...
export type TokenBalanceUpdate = {
  isPending?: boolean;
  balanceType: "wallet" | "tradingAccount";
  balance?: bigint;
  diff?: bigint;
  clearTimerId?: number;
};
...
type TokensBalancesContextType = {
  ...
  resetTokensBalancesUpdates: (tokenAddresses: string[], balanceType: "wallet" | "tradingAccount") => void;
};
...
export function TokensBalancesContextProvider({ children }: PropsWithChildren) {
  ...
  const resetTokensBalancesUpdates = useCallback((tokenAddresses: string[], balanceType: "wallet" | "tradingAccount") => {
...
export function useUpdatedTokensBalances<T extends TokenBalancesData | TokensData>(
  ...
  balanceType: T extends TokensData ? undefined : "wallet" | "tradingAccount"
): T | undefined {
...
const applyBalanceUpdate = <T extends TokenBalancesData | TokensData>(
  ...
  if (typeof nextBalancesData[tokenAddress] === "bigint") {
    ...
    if (tokenData.tradingAccountBalance !== undefined) {
      tokenData.tradingAccountBalance = updateTokenBalance(balanceUpdate, tokenData.tradingAccountBalance, "tradingAccount");
    }

    tokenData.balance = tokenData.isTradingAccount ? tokenData.tradingAccountBalance! : tokenData.walletBalance!;
    nextBalancesData[tokenAddress] = tokenData as T[Extract<keyof T, string>];
  }
