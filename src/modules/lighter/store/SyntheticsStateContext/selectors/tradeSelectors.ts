...
export const makeSelectIncreasePositionAmounts = ({
  ...
  return createSelector((q) => {
    ...
    // In API trading mode, if a token is missing from tokensData, fall back to marketsInfoData.
    // This covers synthetic tokens such as "synth-sol" that are created from backend market data.
    if (!indexToken && indexTokenAddress && indexTokenAddress.startsWith("synth-") && marketsInfoData) {
    ...
    // If marketInfo is missing, fall back to the indexTokenAddress lookup.
    // This covers synthetic-token flows where marketAddress is not set explicitly.
    if (!marketInfo && indexTokenAddress && marketsInfoData) {
      // Find the market by indexTokenAddress for synthetic tokens such as "synth-sol".
