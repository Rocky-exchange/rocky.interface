...
import type { ReaderOrderInfoStructOutput } from "typechain-types/SyntheticsReader";
...
function isAddressEqual(a: string, b: string) {
...
function parseResponse(res: MulticallResult<ReturnType<typeof buildUseOrdersMulticall>>) {
  ...
  return {
    ...
    orders: orders.map((order, i) => {
      ...
      const orderData = order.order as ReaderOrderInfoStructOutput["order"];
