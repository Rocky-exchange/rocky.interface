...
export function getDecreaseError(p: {
  ...
  if (isTrigger) {
    ...
    if (existingPosition?.liquidationPrice && existingPosition.liquidationPrice !== MAX_UINT_256) {
...
export function getEditCollateralError(p: {
  ...
  if (nextLiqPrice !== undefined && position?.markPrice !== undefined) {
    if (position?.isLong && nextLiqPrice < MAX_UINT_256 && position?.markPrice < nextLiqPrice) {
