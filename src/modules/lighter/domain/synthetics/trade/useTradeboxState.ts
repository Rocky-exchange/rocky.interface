...
import { createGetMaxLongShortLiquidityPool } from "@/modules/lighter/store/SyntheticsStateContext/selectors/tradeboxSelectors";
...
type StoredTradeOptions = {
  ...
  isFromTokenTradingAccount: boolean;
};
...
const INITIAL_SYNTHETICS_TRADE_OPTIONS_STATE: StoredTradeOptions = {
  ...
  isFromTokenTradingAccount: false,
};
...
export function useTradeboxState(
  ...
  // The dex tradebox path was removed; only the API-backed flow remains, so
  // any branching that used to depend on the page-type flag is gone.
  ...
  const setStoredOptionsOnChain = useCallback(
    ...
    [chainId, routePrefix, latestEnabled, latestHistory]
  );
    ...
    function handleChainChange() {
      ...
      if (raw) {
        ...
        if (saved.tokens.indexTokenAddress && availableIndexTokensAddresses.includes(saved.tokens.indexTokenAddress)) {
          // Force fromTokenAddress to be USDT and use tradingAccount balance.
          try {
          ...
          saved.isFromTokenTradingAccount = true;
      ...
      // Default fromTokenAddress is USDT; tradingAccount balance is always
      // sourced from the /balances API.
      ...
      const defaultIsFromTokenTradingAccount = true;
      ...
      setStoredOptionsOnChain({
        ...
        isFromTokenTradingAccount: defaultIsFromTokenTradingAccount,
      });
      setSyncedChainId(chainId);
    },
    [
  ...
  const setStoredOptions = useCallback(
    (args: SetStateAction<StoredTradeOptions>) => {
      setStoredOptionsOnChain((oldState) => {
        ...
        if (newState && (newState.tradeType === TradeType.Long || newState.tradeType === TradeType.Short)) {
          // We use synthetic addresses from API rather than local config, so
          // allowedPayTokens / allowedMarkets are empty — fallbackPositionTokens
          // accepts the synthetic "synth-*" addresses directly.
    ...
    [setStoredOptionsOnChain, chainId, srcChainId, marketsInfoData]
  );
  ...
  // Separate TP/SL input values for API trading mode
  ...
  const isFromTokenTradingAccount = Boolean(storedOptions?.isFromTokenTradingAccount);
  ...
  const setIsFromTokenTradingAccount = useCallback(
    (isFromTokenTradingAccount: boolean) => {
      setStoredOptions((oldState) => ({ ...oldState, isFromTokenTradingAccount }));
    },
    [setStoredOptions]
  );
  ...
  useEffect(
    function updateSwapTokens() {
      ...
      const canFromTokenBeTradingAccount = isSettlementChain(chainId);
      // tradingAccount balance is always required (it's the API-backed source).
      const mustFromTokenBeTradingAccount = true;
      const needFromIsTradingAccountUpdate =
        (!canFromTokenBeTradingAccount && isFromTokenTradingAccount) ||
        (mustFromTokenBeTradingAccount && !isFromTokenTradingAccount);

      if (needFromAddressUpdate) {
        setFromTokenAddress(swapTokens[0].address);
      }

      if (needFromIsTradingAccountUpdate) {
        setIsFromTokenTradingAccount(true);
      }
  ...
  useEffect(
    function fallbackIsFromTokenTradingAccount() {
      if (expressOrdersEnabled) {
        return;
      }

      if (isFromTokenTradingAccount && !expressOrdersEnabled) {
        setIsFromTokenTradingAccount(false);
      }
    },
    [expressOrdersEnabled, isFromTokenTradingAccount, setIsFromTokenTradingAccount]
  );
...
function fallbackPositionTokens({
  ...
  // We only collateralise in USDT, so accept that address even though
  // allowedPayTokens is empty in the API-backed flow.
  let isNextPayTokenValid = nextPayTokenAddress && allowedPayTokensSet.has(nextPayTokenAddress);
  if (nextPayTokenAddress && !isNextPayTokenValid) {
    try {
      const usdtToken = getTokenBySymbol(chainId, "USDT");
      if (usdtToken && nextPayTokenAddress.toLowerCase() === usdtToken.address.toLowerCase()) {
        isNextPayTokenValid = true;
      }
    } catch (e) {
      // USDT lookup failure → fall through to the regular validity check.
    }
  }
  // Synthetic "synth-*" addresses come from the API and are absent from local
  // config, so accept them even when missing from allowedIndexTokens / marketsMap.
  const isNextIndexTokenValid =
    nextIndexTokenAddress &&
    (allowedIndexTokens.has(nextIndexTokenAddress) || nextIndexTokenAddress.startsWith("synth-"));
  const isNextMarketTokenValid =
    ...
    (marketsMap[nextMarketTokenAdress]?.marketTokenAddress === nextMarketTokenAdress ||
      nextMarketTokenAdress.startsWith("synth-"));
  // tradingAccount balance is the only valid funding source in the API flow.
  const isNextPayTokenSourceValid = nextState.isFromTokenTradingAccount;
  ...
  const fallbackIndexTokenAndMarket = (fallbackIndexToken?: string) => {
    const isSyntheticToken = !!fallbackIndexToken?.startsWith("synth-");
    ...
    const isValidMarketToken =
      ...
      (marketsMap[fallbackMarketTokenAddress]?.marketTokenAddress === fallbackMarketTokenAddress ||
        fallbackMarketTokenAddress.startsWith("synth-"));
    ...
    // For synthetic tokens with no resolved market, derive the synthetic
    // market address from the symbol (e.g. "synth-sol" → "synth-SOL-USD").
    if (isSyntheticToken && !fallbackMarketToken) {
      const symbolMatch = fallbackIndexToken.match(/^synth-(.+)$/i);
      if (symbolMatch) {
        fallbackMarketToken = `synth-${symbolMatch[1].toUpperCase()}-USD`;
      }
    }
  ...
  if (!isNextPayTokenValid) {
    // Default to USDT — the only collateral the API flow supports.
    ...
    if (!fallbackPayTokenAddress && nextPayTokenAddress) {
      try {
        ...
        // If USDT lookup fails just skip — caller already has a sensible default.
      }
    }
  ...
  if (!isNextIndexTokenValid) {
    ...
    if (nextIndexTokenAddress) {
      // Synthetic addresses aren't in local config; resolve via allowedMarkets.
      ...
      if (matchingMarket) {
        ...
        const symbolMatch = nextIndexTokenAddress.match(/^synth-(.+)$/i);
        if (symbolMatch) {
          const symbol = symbolMatch[1].toUpperCase();
          const similarMarket = allowedMarkets.find((m) => m.indexToken?.symbol?.toUpperCase() === symbol);
          if (similarMarket) {
  ...
  if (!isNextPayTokenSourceValid) {
    nextState = produce(nextState, (draft) => {
      draft.isFromTokenTradingAccount = true;
    });
  }

  let isFallbackPayTokenValid =
    nextState.tokens.fromTokenAddress && allowedPayTokensSet.has(nextState.tokens.fromTokenAddress);
  if (nextState.tokens.fromTokenAddress && !isFallbackPayTokenValid) {
    try {
      const usdtToken = getTokenBySymbol(chainId, "USDT");
      if (usdtToken && nextState.tokens.fromTokenAddress.toLowerCase() === usdtToken.address.toLowerCase()) {
  ...
  const isFallbackIndexTokenValid =
    ...
    (allowedIndexTokens.has(nextState.tokens.indexTokenAddress) ||
      nextState.tokens.indexTokenAddress.startsWith("synth-"));
