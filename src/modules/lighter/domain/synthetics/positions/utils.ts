...
export function formatAcceptablePrice(acceptablePrice?: bigint, opts: { visualMultiplier?: number } = {}) {
  if (acceptablePrice !== undefined && (acceptablePrice == 0n || acceptablePrice >= MAX_INT_256)) {
    return "NA";
  }
...
export function formatLeverage(leverage?: bigint) {
  ...
  const raw = formatAmount(leverage, 4, 2);
  const asNumber = Number(raw);

  if (Number.isNaN(asNumber)) return `${raw}x`;

  // 将特别大的杠杆（例如 10000）格式化为 10kx 风格
  const rounded = Math.round(asNumber);
  if (rounded >= 1000 && Number.isInteger(rounded / 1000)) {
    return `${rounded / 1000}kx`;
  }
