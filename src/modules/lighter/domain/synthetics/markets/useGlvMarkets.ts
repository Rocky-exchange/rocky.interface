  ...
  const { data: glvData, isLoading: isLoadingGlvsInfo } = useMulticall<{}, GlvInfoData | undefined>(
    ...
    "useGlvMarketsInfos",
    {
      ...
      request: () => {
        ...
        const dataStoreAddressForRequest = dataStoreAddress;
        const glvReaderAddressForRequest = glvReaderAddress;

        if (!dataStoreAddressForRequest || !glvReaderAddressForRequest) {
          throw new Error("GLV contracts are not configured");
        }
        ...
        const request = glvs.reduce((acc, { glv, markets }) => {
          ...
          acc[glv.glvToken + "-prices"] = {
            contractAddress: glvReaderAddressForRequest,
            abiId: "GlvReader",
            calls: {
              glvTokenPriceMin: {
          ...
          acc[glv.glvToken + "-info"] = {
            contractAddress: dataStoreAddressForRequest,
            abiId: "DataStore",
            calls: {
