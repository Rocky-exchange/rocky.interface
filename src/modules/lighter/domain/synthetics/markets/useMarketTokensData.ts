...
import { selectGlvInfo, selectGlvs } from "@/modules/lighter/store/SyntheticsStateContext/selectors/globalSelectors";
...
export function useMarketTokensDataRequest(
  ...
  // Only enable for routes that still surface GM / GLV token prices.
  const isEnabledRoute =
    typeof window !== "undefined" &&
    ["/earn"].some((route) => window.location.pathname.startsWith(route));

  const isDataLoaded = isEnabledRoute && tokensData && marketsAddresses?.length && isGlvTokensLoaded;
