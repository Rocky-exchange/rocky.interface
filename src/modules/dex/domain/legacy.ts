...
export function useTotalGmxSupply() {
  ...
  const { data: gmxSupply, mutate: updateGmxSupply, error } = useSWR(gmxSupplyUrlArbitrum, {
    // @ts-ignore
    fetcher: async (url) => {
      const res = await fetch(url);
      if (!res.ok) {
        throw new Error(`Failed to fetch GMX supply: ${res.status} ${res.statusText}`);
      }
      const text = await res.text();
      // Validate that the response is a number string, not HTML
      const trimmed = text.trim();
      if (!trimmed || isNaN(Number(trimmed)) || trimmed.startsWith('<')) {
        throw new Error(`Invalid GMX supply response: expected number, got ${trimmed.substring(0, 50)}`);
      }
      return trimmed;
    },
  });

  // Only convert to bigint if we have valid numeric data
  ...
  if (gmxSupply && !error) {
    try {
      ...
      console.warn("[useTotalGmxSupply] Failed to convert GMX supply to bigint:", e);
      total = undefined;
    }
  }
