...
export function getIncreaseError(p: {
  ...
  // For x10000 route, prioritize "Enter an amount" over "Select a market"
  // In x10000 route, always show "Enter an amount" when market is not selected
  const hasNoMarket = !marketInfo || !indexToken;
  
  if (hasNoMarket) {
    // In x10000 route, show "Enter an amount" instead of "Select a market"
    return [t`Enter an amount`];
  }
...
export function getGmSwapError(p: {
  ...
  const glvTooltipMessage = t`The buyable cap for the pool ${marketInfo.name} using the pay token selected is reached. Please choose a different pool, reduce the buy size, or pick a different composition of tokens.`;
  ...
  if (isDeposit) {
    ...
    if (glvInfo) {
      ...
      if (maxLongExceeded || maxShortExceeded) {
        return [t`Max buyable amount reached`, glvTooltipMessage];
      }
    } else {
      const mintableInfo = getMintableMarketTokens(marketInfo, marketToken);
    ...
    if (glvInfo) {
      ...
      if (marketTokenUsd !== undefined && (mintableGmUsd < marketTokenUsd || maxMintableInMarketUsd < marketTokenUsd)) {
        return [
          ...
          longToken?.symbol === "GM"
            ? t`The buyable cap for the pool ${marketInfo.name} in ${getGlvDisplayName(glvInfo)} [${getMarketPoolName(glvInfo)}] has been reached. Please reduce the buy size, pick a different token, or shift the tokens to a different pool and try again.`
            : t`The buyable cap for the pool ${marketInfo.name} in ${getGlvDisplayName(glvInfo)} [${getMarketPoolName(glvInfo)}] has been reached. Please choose a different pool or reduce the buy size.`,
        ];
      }
    }
  } else {
