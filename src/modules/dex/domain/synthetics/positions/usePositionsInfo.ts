...
export function usePositionsInfoRequest(
  ...
  return useMemo(() => {
    ...
    const positionsInfoData = Object.keys(positionsData).reduce((acc: PositionsInfoData, positionKey: string) => {
      ...
      // Try marketsData first, fallback to marketInfo for synthetic markets (x10000 mode)
      ...
      // In x10000 mode, collateralAmount is already in USDT (6 decimals), so we need to convert it to USD (30 decimals)
      ...
      const isX10000Mode = position.originalPositionId !== undefined;
      const collateralUsd = isX10000Mode
      ...
      // In x10000 mode, use API returned PNL directly instead of recalculating
      // Recalculating can cause precision issues or incorrect values
      const pnl = isX10000Mode
        ? position.pnl
        : marketInfo
          ? getPositionPnlUsd({
      ...
      // In x10000 mode, use backend's stored leverage directly to avoid
      // fee-deducted collateral causing inflated leverage display
      const leverage = isX10000Mode && position.originalLeverage
      ...
      const leverageWithPnl = isX10000Mode && position.originalLeverage
        ? position.originalLeverage
        : getLeverage({
            sizeInUsd: position.sizeInUsd,
            collateralUsd: collateralUsd,
      ...
      const leverageWithoutPnl = isX10000Mode && position.originalLeverage
        ? position.originalLeverage
        : getLeverage({
            sizeInUsd: position.sizeInUsd,
            collateralUsd: collateralUsd,
