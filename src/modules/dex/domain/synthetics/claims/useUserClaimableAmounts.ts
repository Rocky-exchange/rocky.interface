...
import { getMarketAddressFromSymbol } from "config/custom/contracts";
...
export default function useUserClaimableAmounts(chainId: ContractsChainId, account?: string): ClaimableAmountsResult {
  ...
  const markets = useMemo(() => {
    ...
    // ETH/USD market used for claimable amounts
    const ethUsdMarketAddress = getMarketAddressFromSymbol(chainId, "ETH-USD");
    return ethUsdMarketAddress && marketsInfo[ethUsdMarketAddress]
      ? [marketsInfo[ethUsdMarketAddress]]
      : [];
  }, [marketsInfo, chainId]);
