...
import { selectTradeboxTradeTypeError } from "@/modules/cex/context/SyntheticsStateContext/selectors/tradeboxSelectorsx10000/selectTradeboxTradeErrors";
...
import { convertTokenAddress, getToken, getTokenBySymbol, getTokenBySymbolSafe, getTokenVisualMultiplier } from "sdk/configs/tokens";
...
import { tradeTypeLabelsx10000 as tradeTypeLabels } from "../tradeboxConstantsx10000";
import { useTradeboxTransactionsx10000 as useTradeboxTransactions } from "./useTradeboxTransactionsx10000";
...
export function useTradeboxButtonStatex10000({
  account,
  ...
  const { isSwap, isIncrease, isLimit, isMarket, isTwap, isTrigger } = tradeFlags;
  ...
  const { authenticate } = useZtdxAuth();
  ...
  const { buttonErrorText, tooltipContent } = useMemo(() => {
    ...
    // Check JWT authentication for x10000 mode
    // If wallet is connected but not authenticated, show Sign In
    let authError: ValidationResult = [undefined];
    if (account && !isAuthenticated(account, chainId)) {
      authError = [t`Sign In`];
    }
    ...
    const buttonErrorText = commonError[0] || authError[0] || tradeError[0] || expressError[0];
  ...
  const onSubmit = useCallback(async () => {
    console.log(" onSubmit button clicked", { account: !!account, signer: !!signer, isSwap, isIncrease });

    // For x10000 mode, we use API-based order submission which doesn't require signer
    // We only need account (wallet address) for API orders, as signature is done via signMessageAsync
    if (!account) {
      console.log(" No account, opening connect modal");
    ...
    // If wallet is connected but not authenticated, trigger sign in flow
    ...
    // For non-API orders (like wrap/unwrap, stake/unstake), we still need signer
    // But for regular orders in x10000 mode, we skip signer check
    if (!isSwap && !isStakeOrUnstake && !isWrapOrUnwrap) {
      // For x10000 API orders, signer is not required
      // Signature will be handled via signMessageAsync in useZtdxOrderSubmit
    } else if (!signer) {
      console.log(" No signer available for non-API operation");
      return;
    }
  ...
  return useMemo(() => {
    ...
    // For x10000 route (centralized trading), convert "Select a market" to "Enter an amount"
    // This matches the expected behavior in ztdx_interface
    let displayErrorText = buttonErrorText;
    if (buttonErrorText === t`Select a market`) {
      displayErrorText = t`Enter an amount`;
    }

    // For x10000 route (centralized trading), if user has entered an amount,
    // ignore "Enter an amount" error and allow button to show normal text
    // Validation will be handled by backend
    // In Trigger mode (TP/SL), user doesn't enter amount - they set trigger prices, so skip amount validation
    const hasEnteredAmount = fromTokenInputValue && fromTokenInputValue.trim() !== "" && parseFloat(fromTokenInputValue) > 0;
    const shouldIgnoreAmountError = (hasEnteredAmount && displayErrorText === t`Enter an amount`) ||
      (isTrigger && displayErrorText === t`Enter an amount`);

    if (displayErrorText && !shouldIgnoreAmountError) {
      return {
        ...commonState,
    ...
    {
      // Only show error text if it's not "Enter an amount" and user has entered a value
      if (displayErrorText && !shouldIgnoreAmountError) {
        submitButtonText = displayErrorText;
      }
...
function NoSwapPathTooltipContent({
  ...
  const makeHandleSwapClick = useCallback(
    (fromTokenSymbol: string, toTokenSymbol: string) => () => {
      ...
      // Use getTokenBySymbolSafe to avoid throwing errors for unsupported tokens
      const fromToken = getTokenBySymbolSafe(chainId, fromTokenSymbol);
      const toToken = getTokenBySymbolSafe(chainId, toTokenSymbol);
      if (fromToken) {
