...
/**
 * SWR cache keys related to ZTDX
 */
const ZTDX_CACHE_KEYS = [
  "ztdx-balances",
  "ztdx-positions",
  "ztdx-orders",
  "x10000-ztdx-balances",
];

/**
 * Clear SWR cache entries for a specific address and/or chain
 */
function clearZtdxCache(options: {
  address?: string;
  chainId?: number;
  onlyChainId?: boolean;
}): void {
  ...
  cacheKeys.forEach((key) => {
    ...
    // Check if this is a ZTDX-related key
    const isZtdxKey = keyArray.some(
      (k) => typeof k === "string" && ZTDX_CACHE_KEYS.includes(k)
    );
